...
local LEAGUE = tostring(data.get("basketball_schedule.league") or "nba")
local TEAM = string.upper(tostring(data.get("basketball_schedule.team") or data.get("sports.default_team") or ""))
local TEAM_ID = tostring(data.get("basketball_schedule.team_id") or data.get("sports.default_team_id") or "")
local COUNT = tonumber(data.get("basketball_schedule.count") or 3) or 3
local SCHEDULE_LIMIT = math.max(1, tonumber(data.get("basketball_schedule.schedule_limit") or 8) or 8)
local SCORE_DAYS = math.max(1, tonumber(data.get("basketball_schedule.score_days") or 7) or 7)
local INCLUDE_LAST_RESULT = tostring(data.get("basketball_schedule.include_last_result") or "1") ~= "0"
local ROTATE_MS = tonumber(data.get("basketball_schedule.rotate_ms") or 4000) or 4000
local TTL_MS = tonumber(data.get("basketball_schedule.ttl_ms") or 10 * 60 * 1000) or (10 * 60 * 1000)
local TEAMS_MAX_BODY = tonumber(data.get("basketball_schedule.teams_max_body") or 65536) or 65536
local SCOREBOARD_MAX_BODY = tonumber(data.get("basketball_schedule.scoreboard_max_body") or 262144) or 262144
local TEAM_SCHEDULE_MAX_BODY = tonumber(data.get("basketball_schedule.team_schedule_max_body") or 196608) or 196608
...
local __boot_ms = tonumber(data.get("basketball_schedule.boot_splash_ms") or data.get("app.boot_splash_ms") or 1200) or 1200
if __boot_ms < 0 then __boot_ms = 0 end
local __boot_name = tostring(data.get("basketball_schedule.app_name") or "Basketball Schedule")
