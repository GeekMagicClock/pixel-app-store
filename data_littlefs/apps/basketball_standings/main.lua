...
local LEAGUE = tostring(data.get("basketball_standings.league") or "nba")
local TEAM = string.upper(tostring(data.get("basketball_standings.team") or data.get("sports.default_team") or ""))
local MODE = tostring(data.get("basketball_standings.mode") or (TEAM ~= "" and "around_team" or "top"))
local AUTO_PAGE = tostring(data.get("basketball_standings.auto_page") or "1") ~= "0"
local ROTATE_MS = tonumber(data.get("basketball_standings.rotate_ms") or 3500) or 3500
local ROWS_PER_PAGE = math.max(1, math.min(4, tonumber(data.get("basketball_standings.rows_per_page") or 4) or 4))
...
local __boot_ms = tonumber(data.get("basketball_standings.boot_splash_ms") or data.get("app.boot_splash_ms") or 5000) or 5000
if __boot_ms < 0 then __boot_ms = 0 end
local __boot_name = tostring(data.get("basketball_standings.app_name") or "Basketball Standings")
