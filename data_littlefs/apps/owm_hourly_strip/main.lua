  ...
  if kind == "geo" then
    if parse_geo(status, body) then
      local wx_url = string.format(
        "http://api.open-meteo.com/v1/forecast?latitude=%s&longitude=%s&hourly=temperature_2m,precipitation_probability,precipitation,weather_code&forecast_days=2&timezone=%s&temperature_unit=%s",
...
local __boot_ms = tonumber(data.get("owm_hourly_strip.boot_splash_ms") or data.get("app.boot_splash_ms") or 5000) or 5000
if __boot_ms < 0 then __boot_ms = 0 end
local __boot_name = tostring(data.get("owm_hourly_strip.app_name") or "Hourly Weather Strip")
