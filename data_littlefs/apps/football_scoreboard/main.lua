...
local LEAGUE = tostring(data.get("football_scoreboard.league") or "nfl")
local TEAM = string.upper(tostring(data.get("football_scoreboard.team") or data.get("game_summary.team") or data.get("sports.default_team") or ""))
local TEAM_ID = tostring(data.get("football_scoreboard.team_id") or data.get("game_summary.team_id") or data.get("sports.default_team_id") or "")
local EVENT_ID = tostring(data.get("football_scoreboard.event_id") or data.get("game_summary.event_id") or "")
local SCOREBOARD_ONLY = tostring(data.get("football_scoreboard.scoreboard_only") or data.get("game_summary.scoreboard_only") or ((SPORT == "soccer" and EVENT_ID == "") and "1" or "0")) == "1"
local USE_DATE_FILTER = tostring(data.get("football_scoreboard.use_date_filter") or data.get("game_summary.use_date_filter") or "1") ~= "0"
local AUTO_PAGE = tostring(data.get("football_scoreboard.auto_page") or data.get("game_summary.auto_page") or "1") ~= "0"
local ROTATE_SEC = num_setting("football_scoreboard.rotate_sec", num_setting("game_summary.rotate_sec", nil))
local REFRESH_SEC = num_setting("football_scoreboard.refresh_sec", num_setting("game_summary.refresh_sec", nil))
local ROTATE_MS = math.floor((ROTATE_SEC and ROTATE_SEC > 0 and ROTATE_SEC or (num_setting("football_scoreboard.rotate_ms", num_setting("game_summary.rotate_ms", 3500)) / 1000)) * 1000)
local TTL_MS = math.floor((REFRESH_SEC and REFRESH_SEC > 0 and REFRESH_SEC or (num_setting("football_scoreboard.ttl_ms", num_setting("game_summary.ttl_ms", 60000)) / 1000)) * 1000)
local MAX_BODY = tonumber(data.get("football_scoreboard.max_body") or data.get("game_summary.max_body") or 512000) or 512000
local SCOREBOARD_MAX_BODY = tonumber(data.get("football_scoreboard.scoreboard_max_body") or data.get("game_summary.scoreboard_max_body") or 458752) or 458752
local SCOREBOARD_MAX_BODY_HARD = 524288
local TIMEOUT_MS = tonumber(data.get("football_scoreboard.timeout_ms") or data.get("game_summary.timeout_ms") or 8000) or 8000
local APP_NAME = tostring(data.get("football_scoreboard.app_name") or "Football Scoreboard")
...
local __boot_ms = tonumber(data.get("football_scoreboard.boot_splash_ms") or data.get("app.boot_splash_ms") or 5000) or 5000
if __boot_ms < 0 then __boot_ms = 0 end
local __boot_name = tostring(data.get("football_scoreboard.app_name") or "Football Scoreboard")
