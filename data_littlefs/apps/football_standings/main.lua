...
local LEAGUE = tostring(data.get("football_standings.league") or "nfl")
local TEAM = string.upper(tostring(data.get("football_standings.team") or data.get("sports.default_team") or ""))
local MODE = tostring(data.get("football_standings.mode") or (TEAM ~= "" and "around_team" or "top"))
local AUTO_PAGE = tostring(data.get("football_standings.auto_page") or "1") ~= "0"
local ROTATE_MS = tonumber(data.get("football_standings.rotate_ms") or 3500) or 3500
local ROWS_PER_PAGE = math.max(1, math.min(4, tonumber(data.get("football_standings.rows_per_page") or 4) or 4))
local TTL_MS = tonumber(data.get("football_standings.ttl_ms") or 10 * 60 * 1000) or (10 * 60 * 1000)
local MAX_BODY = tonumber(data.get("football_standings.max_body") or 393216) or 393216
local TIMEOUT_MS = tonumber(data.get("football_standings.timeout_ms") or 8000) or 8000
local APP_NAME = tostring(data.get("football_standings.app_name") or "Football Standings")
...
local __boot_ms = tonumber(data.get("football_standings.boot_splash_ms") or data.get("app.boot_splash_ms") or 5000) or 5000
if __boot_ms < 0 then __boot_ms = 0 end
local __boot_name = tostring(data.get("football_standings.app_name") or "Football Standings")
