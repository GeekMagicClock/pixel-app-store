...
local LEAGUE = tostring(data.get("football_schedule.league") or "nfl")
local TEAM = string.upper(tostring(data.get("football_schedule.team") or data.get("sports.default_team") or ""))
local TEAM_ID = tostring(data.get("football_schedule.team_id") or data.get("sports.default_team_id") or "")
local COUNT = tonumber(data.get("football_schedule.count") or 3) or 3
local SCHEDULE_LIMIT = math.max(1, tonumber(data.get("football_schedule.schedule_limit") or 8) or 8)
local SCORE_DAYS = math.max(1, tonumber(data.get("football_schedule.score_days") or 7) or 7)
local INCLUDE_LAST_RESULT = tostring(data.get("football_schedule.include_last_result") or "1") ~= "0"
local ROTATE_MS = tonumber(data.get("football_schedule.rotate_ms") or 4000) or 4000
local TTL_MS = tonumber(data.get("football_schedule.ttl_ms") or 10 * 60 * 1000) or (10 * 60 * 1000)
local TEAMS_MAX_BODY = tonumber(data.get("football_schedule.teams_max_body") or 65536) or 65536
local SCOREBOARD_MAX_BODY = tonumber(data.get("football_schedule.scoreboard_max_body") or 262144) or 262144
local TEAM_SCHEDULE_MAX_BODY = tonumber(data.get("football_schedule.team_schedule_max_body") or 196608) or 196608
local TEAMS_CACHE_TTL_MS = tonumber(data.get("football_schedule.teams_cache_ttl_ms") or (24 * 60 * 60 * 1000)) or (24 * 60 * 60 * 1000)
local TIMEOUT_MS = tonumber(data.get("football_schedule.timeout_ms") or 30000) or 30000
local ERR_RETRY_MS = tonumber(data.get("football_schedule.err_retry_ms") or 15000) or 15000
local APP_NAME = tostring(data.get("football_schedule.app_name") or "Basketball Schedule")
...
local function dbg(msg)
  print("[football_schedule] " .. tostring(msg or ""))
end
...
local function teams_cache_key(prefix)
  return string.format("football_schedule.%s.%s.%s", tostring(prefix), tostring(SPORT), tostring(LEAGUE))
end
