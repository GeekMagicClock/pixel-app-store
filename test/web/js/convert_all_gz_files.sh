...
mv cropper.min.js.gz cropper_min.js.gz
mv jquery.min.js.gz jquery_min.js.gz
# 在当前目录中查找所有 .html.gz 文件
for gz_file in ./*.js.gz; do
  # 去掉路径的 .html.gz 扩展名并将斜杠替换为下划线以创建数组名称
  file_basename="$(basename "${gz_file%.js.gz}")"
  array_name="${file_basename//\//_}_js"
