...
local function cfg_refresh_ms()
  local n = tonumber(data.get("aqi_now.refresh_ms") or data.get("aqi_now.refresh_interval_ms") or DEFAULT_REFRESH_MS) or DEFAULT_REFRESH_MS
...
local function start_aqi_request()
  ...
  local url = string.format(
    "http://air-quality-api.open-meteo.com/v1/air-quality?latitude=%s&longitude=%s&hourly=us_aqi,pm2_5,pm10,nitrogen_dioxide,ozone&forecast_days=2&timezone=auto",
    tostring(state.lat), tostring(state.lon)
  )
  local ttl_ms = cfg_refresh_ms()
  local id, body, age_ms, err = net.cached_get(url, ttl_ms, 7000, 12288)
  if err then
    state.err = err
    return
  end
